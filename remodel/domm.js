      ...
      const extractedData = list.map((product) => {
        ...
        const images = productImages
          ? {
              ...
              attributes: {
                picture:
                  productImages.attributes &&
                  productImages.attributes.picture &&
                  productImages.attributes.picture.length > 0
                    ? [
                        productImages.attributes.picture[
                          productImages.attributes.picture.length - 1
                        ],
                      ]
                    : [],
              },
...
async function processUrlsFromFile(filePath) {
  try {
    ...
    const results = await Promise.all(urls.map((url) => parseWebsite(url)));
