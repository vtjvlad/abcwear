...
function updateProgressBar(processed, total, times) {
  ...
  process.stdout.write(
    `Progress [${bar}] ${percent.toFixed(2)}% (${processed}/${total}) ETA: ${etaSeconds === 'N/A' ? 'N/A' : formatTime(etaSeconds)}`
  );
...
async function parseWebsites() {
  ...
  for (const chunk of urlChunks) {
    ...
    await fs.writeFile('b3f6_nikeIMG.json', JSON.stringify(results, null, 2), 'utf-8');
  }
