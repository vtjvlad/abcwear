...
async function translateBatchWithRetry(openai, texts, retries = 3, delay = 1000) {
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      console.log(
        `Попытка ${i + 1} не удалась для пакета, повтор через ${delay}мс:`,
        error.message
      );
...
async function translateContent(inputFilePath, outputFilePath) {
  const openai = new OpenAI({
    apiKey:
      'dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd', // Замените на ваш API ключ
  });
